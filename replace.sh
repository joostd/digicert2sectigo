...
[ -f enroll-$order.json ] || jq -n --rawfile csr $order.csr --arg email "$email" --arg comment "replacement for digicert order $order" -f enroll-template.jq > enroll-$order.json
...
# collect certificate
[ -f cert-$order.pem ] || jq -r .sslId order-$order.json | xargs -IN curl -sH @sectigo-headers https://cert-manager.com/api/ssl/v1/collect/N/pem > cert-$order.pem
openssl crl2pkcs7 -certfile cert-$order.pem -nocrl | openssl pkcs7 -print_certs -noout
